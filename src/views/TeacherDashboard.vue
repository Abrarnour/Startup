...
import { BookOpen, Calendar, Users, TrendingUp, AlertCircle, Lock } from 'lucide-vue-next'
...
const handleUploadSuccess = async () => {
  ...
  if (selectedCourseForUpload.value) {
    try {
      const mats = await api.getCourseMaterials(selectedCourseForUpload.value)
      materialCounts.value[selectedCourseForUpload.value] = Array.isArray(mats) ? mats.length : 0
    } catch (e) {}
  }
}
...
const loadMaterialsCounts = async () => {
  for (const course of courses.value) {
    try {
      const mats = await api.getCourseMaterials(course.id)
      materialCounts.value[course.id] = Array.isArray(mats) ? mats.length : 0
    } catch (e) {
